...
function TabelaFinanceira({nome, financeiro,}: {nome: string; financeiro: Financeiro;}) {
  ...
  return (
    <div className="w-full max-w-md border border-[#8C7B62]/20 rounded-sm bg-black/[0.03] overflow-hidden">
      <div className="bg-[#8C7B62]/10 border-b border-[#8C7B62]/20 px-6 py-4">
        <h3 className="text-[#1E1C18] font-serif text-lg tracking-wide">
          {nome}
        </h3>
      </div>
      <dl className="divide-y divide-[#8C7B62]/10">
        {linhas.map((linha) => (
          <div
            key={linha.label}
            className="flex items-center justify-between gap-4 px-6 py-3"
          >
            <dt className="text-xs tracking-[0.1em] uppercase font-light text-[#1E1C18]/60">
              {linha.label}
            </dt>
            <dd className="text-[#8C7B62] font-serif text-base whitespace-nowrap">
              {linha.valor}
            </dd>
          </div>
        ))}
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
      {kpis.map((kpi, i) => (
        <div
          key={kpi.label}
          className="scroll-anim fade-in-up flex flex-col items-center justify-center p-8 border border-[#8C7B62]/20 rounded-sm bg-black/[0.03] hover:bg-black/[0.05] transition-colors"
          style={{ transitionDelay: `${i * 0.2}s` }}
        >
          <span className="text-3xl md:text-4xl font-serif text-[#8C7B62] mb-3 text-center">
            {kpi.valor}
          </span>
          <span className="text-xs tracking-[0.15em] uppercase font-light text-[#1E1C18]/70 text-center">
            {kpi.label}
          </span>
        </div>
      ))}
  ...
  return (
    <div className="flex flex-col gap-10">
      ...
      <div className="scroll-anim fade-in-up text-center">
        ...
        <h3 className="text-2xl md:text-3xl font-serif text-[#1E1C18] mb-8">
          KPIs — {unidade.nome}
        </h3>
      </div>
...
function Secao({ secao }: { secao: Secao }) {
  ...
  return (
    <section
      id={secao.id}
      className="w-full py-24 md:py-32 border-b border-[#8C7B62]/10"
    >
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="scroll-anim fade-in-up mb-16 max-lg:text-center">
          ...
          <h2 className="text-4xl md:text-5xl lg:text-6xl font-serif text-[#1E1C18]">
            {secao.titulo}
          </h2>
        </div>
    ...
    >
      <div className="max-w-3xl mx-auto px-6 flex flex-col items-center text-center gap-10">
        ...
        <div className="scroll-anim fade-in-up">
          ...
          <h2 className="text-2xl md:text-3xl lg:text-4xl font-serif text-[#1E1C18]">
            {rebranding.titulo}
          </h2>
        </div>
        {/* descrição */}
        <p className="scroll-anim fade-in-up text-[#1E1C18]/80 font-light text-xl leading-relaxed">
  ...
  return (
    <section className="relative w-full min-h-[calc(100vh-96px)] flex items-center justify-center overflow-hidden py-6">
      ...
      <div className="relative z-10 w-full max-w-4xl mx-auto px-6 text-center flex flex-col items-center gap-8">
        ...
        <h1 className="scroll-anim fade-in-up text-2xl md:text-3xl lg:text-4xl font-serif text-[#1E1C18] leading-tight">
          {resumoExecutivo.titulo}
        </h1>
        <p className="scroll-anim fade-in-up text-[#1E1C18]/70 font-light leading-relaxed text-base md:text-xl max-w-2xl">
          {resumoExecutivo.texto}
        </p>

        <a
          href={`#${secoes[0].id}`}
          className="scroll-anim fade-in-up inline-flex items-center border border-[#8C7B62]/40 text-[#8C7B62] text-xs tracking-[0.2em] uppercase px-8 py-4 rounded-sm hover:bg-[#8C7B62]/10 transition-colors"
        >
          Ver Unidades
        </a>
      </div>
...
function Header() {
  return (
    <header className="sticky top-0 z-50 w-full bg-[#14120F] border-b border-[#8C7B62]/10">
      <div className="max-w-6xl mx-auto px-6 py-5 flex items-center justify-between max-lg:justify-center">
        <div className="text-center">
          <h1 className="text-2xl font-serif tracking-wide text-[#F5F2EC]">VILA DA GRAÇA</h1>
          <p className="text-2xl font-serif text-[#F5F2EC]">A SUA VILA</p>
        </div>
        <nav className="hidden min-lg:flex gap-8 text-xs tracking-[0.2em] uppercase text-[#F5F2EC]/70">
          {secoes.map((s) => (
            <a
              key={s.id}
              href={`#${s.id}`}
              className="hover:text-[#8C7B62] transition-colors"
            >
              {s.titulo}
            </a>
          ))}
...
function Footer() {
  return (
    <footer className="w-full py-16 border-t border-[#8C7B62]/10">
      <div className="max-w-6xl mx-auto px-6 flex flex-col items-center gap-8">
        <p className="text-[#8C7B62] tracking-[0.3em] text-xs font-bold uppercase">
          Contato
        </p>
        ...
        <div className="flex flex-wrap items-center justify-center gap-x-10 gap-y-6">
          {contato.emails.map((email) => (
            <a
              key={email}
              href={`mailto:${email}`}
              className="flex items-center gap-3 text-[#1E1C18]/80 hover:text-[#8C7B62] transition-colors"
            >
              <Mail className="w-5 h-5 text-[#8C7B62]" strokeWidth={1.5} />
          ...
          {contato.telefones.map((tel) => (
            <a
              ...
              className="flex items-center gap-3 text-[#1E1C18]/80 hover:text-[#8C7B62] transition-colors"
            >
              <Phone className="w-5 h-5 text-[#8C7B62]" strokeWidth={1.5} />
              <span className="text-sm font-light">{tel}</span>
            </a>
          ))}
        </div>

        <p className="text-[#1E1C18]/40 text-xs font-light tracking-wide">
...
export default function Home() {
  return (
    <div className="bg-[#bdbcbc] min-h-screen">
      <Header />
      <Hero />
      <main>
        {secoes.map((secao) => (
